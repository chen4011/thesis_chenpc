...
% 實驗設定：T pose、拍手等等姿勢
% 說明我這一段要講我用了哪些工具的前言
本研究用於量測人體姿態的工具包含兩台 iPhone 手機、Xsens、棋盤格校正版，各自的功能如下：
兩台手機用於記錄人體姿態的影像資料，擺放於特定位置，拍攝受試者的全身影像；
Xsens 用於記錄人體各肢段及棋盤格校正板的朝向資訊，共使用 10+1 顆 IMU 進行量測；
棋盤格校正版用於轉換影像座標系至全域座標系，以建立影像座標系與全域座標系間的關係。
另外，需要求受試者穿著黑色長袖上衣及黑色長褲，長度盡可能遮蓋住手腕及腳踝。


\subsubsection{位置資訊量測工具 - 相機及擺放位置}
% 硬體介紹；使用相機及擺放位置（平面圖和受試者的移動範圍，用上視表示），幀率(frame rate)為 60 Hz，校正版規格、黑色長秀長庫
...
另一顆 IMU 黏貼於棋盤格校正版，量測棋盤格校正板的初始朝向，用以轉換影像座標系至全域座標系，如圖~\ref{ch3_fig_cbimu} 所示，
...
\begin{figure}[!ht]
   ...
   \begin{minipage}{.5\textwidth}
     ...
     \caption*{(d)}
   \end{minipage}
   \caption[座標系定義 (a)影像座標系 (b)相機座標系 (c)骨架座標系 (d)IMU 座標系]{座標系定義 (a)影像座標系 (b)相機座標系 (c)骨架座標系 (d)IMU 座標系}
   \label{ch3_fig_frame}
\end{figure}
...
\begin{figure}[!ht]
   \centering
   \includegraphics[width=14cm]{figure/ch3_fig_1to7cam.png}
