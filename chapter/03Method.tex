...
本研究使用 OpenPose 辨識出每一幀照片中，受試者的骨盆於該幀影像中的平面位置 (即像素座標)。
...
本研究共涉及六種座標系，可分為影像系統及 IMU 感測器系統兩大部分，兩部分的共同座標系為全域座標系。
影像系統涉及三種座標系，分別為校正版座標系、相機座標系及全域座標系；
IMU 感測系統則涉及四種座標系，分別為骨骼座標系、感測器座標系、IMU local 座標系、全域座標系。
...
% 各座標系間的關係介紹
% TODO:校正版座標系和相機座標系的關係
在影像系統座標系定義與轉換的部分，本研究使用關係式~\ref{ch3_equ_image_rotation_matrix}，
將位置點由以全域座標系為表示方式轉換至以相機座標系為表示方式。
\begin{equation}
   R^{cam}_{enu} = R^{cam}_{cb}(R^{enu}_{cb})^{-1}
   \label{ch3_equ_image_rotation_matrix}
\end{equation}

其中，$R^{cam}_{enu}$ 為將影像系統所拍攝到的位置點由全域座標系轉換至相機座標系之旋轉矩陣，

在 IMU 感測器系統座標系定義與轉換的部分，本研究參考~\cite{malleson2017real}所提出之關係式~\ref{ch3_equ_imu_rotation_matrix}，
...
其中，$R^g_{bi}$ 為將 IMU 所附著的骨骼朝向由骨骼座標系轉換至全域座標系之旋轉矩陣，
即圖~\ref{ch3_fig_coordinate_trans}中，從黑色座標系轉換至綠色座標系之旋轉矩陣；
通過將 $R^g_{bi}$ 乘以骨骼座標系中的骨骼朝向，可以將這些座標轉換為以全域座標系表示的骨骼朝向；
...
$R_{ig}$ 為 IMU local 座標系轉換至全域座標系之旋轉矩陣；
即圖~\ref{ch3_fig_coordinate_trans}中，從藍色座標系轉換至綠色座標系之旋轉矩陣；
通過將 $R_{ig}$ 乘以 IMU local 座標系中的骨骼朝向，可以將這些座標轉換為以全域座標系表示的骨骼朝向。
...
\subsubsection{全域座標 - 校正板座標 - 相機座標}
%全域座標 - 校正板座標 - 相機座標旋轉矩陣
板到全域的轉換矩陣
板到相機的轉換矩陣

\subsubsection{感測器座標 - 骨骼座標旋轉矩陣}
% IMU - 骨骼座標轉換矩陣計算介紹
...
經過上一步 $(R_{ib})^{-1}$ 的轉換，已將骨骼朝向轉換為以附著其上的 IMU 感測器座標系為表示方式，
...
每一 .txt 檔案中的 q0、q1、q2、q3 即為將朝向旋轉成以 IMU local 座標系為表示方式的旋轉矩陣。

\subsubsection{IMU local - 全域座標旋轉矩陣}
% IMU - 全域座標轉換矩陣計算介紹
% 解釋表示方法是以哪個座標系去看哪個座標系，然後解釋計算方法
經過上一步 $R_i$ 的轉換，已將骨骼朝向轉換為以統一的 IMU local 感測器座標系為表示方式，
接著，為方便後續與影像辨識而得的位置資訊進行融合，需要進一步將骨骼朝向轉換至位置資訊所在之座標系。
IMU local - 全域座標旋轉矩陣 $R_{ig}$ 包含兩個階段的旋轉，
