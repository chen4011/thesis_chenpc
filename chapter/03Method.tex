...
% FIXME:補相機在空間中的位置圖(簡略的畫一下空間尺度就好，詳細的尺度放在室內實驗環境及尺寸)，還有要加鏡頭資訊
...
\begin{figure}[!ht]
   \centering
   \includegraphics[width=8cm]{figure/ch3_fig_cbimu.png}
...
動作流程為：兩手打直張開呈現 T pose 維持 1~2 秒，然後迅速合攏雙手並拍手，最後維持合掌姿勢 1~2 秒。
...
影像系統涉及三種座標系，分別為圖像座標系 (checkboard coordinate system, cb)、
...
並使用 Pose2Sim ~\cite{Pagnon_2021_Robustness}~\cite{Pagnon_2022_Accuracy}~\cite{Pagnon_2022_JOSS} 提供的校正工具進行外部參數及內部參數計算，
...
% FIXME:可以從增加實驗架設方便性方面作為開頭著手，然後這邊應該可以算是解釋為何我要用兩台相機，所以位置可能需要改到第三章的最一開始
