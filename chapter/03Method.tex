...
% 軟體介紹；xsens，講使用的軟體 MT manager 還有分析出朝向和加速度應用的模型，還有用了幾顆 IMU ，分別擺在身體的哪裡
...
或是輸出純文字檔案 (JSON，XML，YAML...)，其中紀錄關鍵點於圖片中的位置及 0~1 的分數。
本研究使用 OpenPose 作為基礎架構，搭配其提供之 BODY_25B 模型進行影像辨識，
共可辨識出 25 個關節點，如圖~\ref{ch3_fig_OpenPose} 所示。

\begin{figure}[!ht]
   \centering
   \includegraphics[width=8cm]{figure/ch3_fig_OpenPose.png}
...
IMU global 座標系則以所在地為原點，y 軸沿子午線指向正北為正，z 軸指向天頂為正，x 軸與 yz 平面垂直，指向正東為正；
% 全域座標系還不知道怎麼定義QQ
\begin{figure}[!ht]
   \centering
   \begin{minipage}{.5\textwidth}
...
\subsubsection{五台相機}
將圖~\ref{ch3_fig_cameraset_totalcap} 中的相機任選三台進行排列組合，共計有 56 種組合方式之估計結果，每一結果皆可計算出 MPJPE，
MPJPE 計算結果如表~\ref{ch3_cameraset_5cam}所示，其中最佳組合方式為相機 \sout{137，其 MPJPE 為 27.1579 (mm)}；
而最差組合方式為相機 \sout{257，其 MPJPE 為 82.8158 (mm)。標準差為 13.8423 (mm)，平均值為 42.9001 (mm)}。
經由平均值可以發現：相較四台相機的姿勢估計結果，五台相機的姿勢估計結果並無明顯提升，進步幅度約為 4 (mm)；
由標準差可以知道五台相機的姿勢估計結果十分集中。
\begin{table}[!ht]
   ...
   >{\columncolor[HTML]{E7E6E6}}c |c}
      ...
      24567 & 30.4360 & 24568 & 30.1827 & 24578 & 29.4012 & 24678 & 30.1792 & 25678 &   \\
      \hline
