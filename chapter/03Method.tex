...
% 軟體介紹；xsens，講使用的軟體 MT manager 還有分析出朝向和加速度應用的模型(在 MT manager)，
% 還有用了幾顆 IMU ，分別擺在身體的哪裡
...
% 然後要提到因為要從骨骼轉到全域，所以這三個轉換矩陣分別要怎麼取，這樣感覺就可以名正言順的把方程式和代號放進來
...
將每一關節點由以骨骼座標系為表示方式轉換至以 IMU global 座標系為表示方式。
...
接著，需要進一步將各自表示的感測器座標系轉換至統一以東北天為定義的 IMU local 座標系。
感測器座標 - IMU local 旋轉矩陣 ($R_i$) 可直接經由 Xsens 系統所提供軟體 (MT manager) 輸出四元數取得。
在 MT manager 中讀入量測時所記錄的 .mtb 檔案，以 \sout{...模型為基礎}，並於 file>export 中的輸出選項勾選 quaternion 後輸出，
...
IMU local - IMU global 旋轉矩陣 $R_{ig}$ 包含兩個階段的旋轉，
第一階段先將於 IMU local 座標系的骨骼朝向轉換至以 IMU global 座標系表示，與天頂及磁北極對齊，
第二階段將於 IMU global 座標系的骨骼朝向轉換至位置資訊所在之座標系，該座標系相對 IMU global 座標系的 x 軸旋轉 $-90^{\circ}$。
在第一階段中，IMU local - IMU global 的轉換與磁偏角 ($\delta$) 有關，
將 IMU local 座標系繞 z 軸旋轉 $\delta$ 即可轉換至 IMU global 座標系。
第二階段再將 IMU global 座標系繞 x 軸旋轉 $-90^{\circ}$，轉換至位置資訊所在之座標系。
